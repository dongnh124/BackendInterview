...
│   ├── bucket-and-object-model.md           Bucket, object, key, metadata, ETag
│   ├── storage-classes.md                   Standard, IA, Glacier, Intelligent-Tiering — so sánh
│   ├── versioning-and-mfa-delete.md         Quản lý phiên bản, xóa có xác thực 2FA
│   ├── presigned-urls.md                    URL có chữ ký tạm thời — cách tạo và dùng
│   └── multipart-upload.md                  Tải lên nhiều phần cho file lớn (>5GB)
│
├── 02-s3-advanced/
│   ├── README.md                            S3 nâng cao — replication, events, batch
│   ├── lifecycle-policies.md                Chính sách vòng đời — tự động hóa chuyển tầng
│   ├── cross-region-replication.md          CRR — Sao chép liên vùng, cấu hình, use cases
│   ├── same-region-replication.md           SRR — Sao chép cùng vùng, compliance
│   ├── s3-object-lock.md                    WORM — Write Once Read Many, Governance vs Compliance
│   ├── s3-event-notifications.md            Thông báo sự kiện — Lambda, SQS, SNS, EventBridge
│   └── s3-batch-operations.md               Thao tác hàng loạt — copy, tag, restore, invoke Lambda
│
├── 03-ebs/
│   ├── README.md                            EBS — Elastic Block Store, tổng quan
│   ├── volume-types.md                      gp2/gp3, io1/io2, st1, sc1 — so sánh chi tiết
│   ├── snapshots-and-lifecycle.md           Ảnh chụp nhanh, DLM — Data Lifecycle Manager
│   ├── ebs-multi-attach.md                  Gắn kết nhiều EC2 — io1/io2 chỉ
│   ├── performance-tuning.md                IOPS, Throughput, Latency — tối ưu hiệu suất
│   ├── encryption-with-kms.md               Mã hóa với KMS — Key Management Service
│   └── ebs-vs-instance-store.md             So sánh lưu trữ bền vững vs tạm thời
│
├── 04-efs/
│   ├── README.md                            EFS — Elastic File System, tổng quan
│   ├── mount-targets-and-access-points.md   NFS mount targets, access points, IAM authz
│   ├── performance-modes.md                 General Purpose vs Max I/O — khi nào dùng
│   ├── throughput-modes.md                  Bursting, Provisioned, Elastic — so sánh
│   ├── efs-intelligent-tiering.md           Phân tầng thông minh — Standard ↔ IA tự động
│   └── efs-vs-ebs-vs-s3.md                  Bảng so sánh đầy đủ — chọn dịch vụ phù hợp
│
├── 05-security/
│   ├── README.md                            Bảo mật lưu trữ — tổng quan
│   ├── iam-policies-for-storage.md          IAM policies cho S3, EBS, EFS — examples
│   ├── s3-bucket-policies.md                Bucket policies, condition keys, cross-account
│   ├── block-public-access.md               Chặn truy cập công khai — cấu hình & best practices
│   ├── encryption-at-rest.md                SSE-S3, SSE-KMS, SSE-C, CSE — so sánh chi tiết
│   ├── encryption-in-transit.md             TLS/HTTPS enforcement, VPC endpoints
│   ├── vpc-endpoints.md                     Gateway endpoint vs Interface endpoint cho S3/EFS
│   └── s3-access-analyzer.md                Phân tích quyền truy cập — phát hiện public access
│
├── 06-cost-optimization/
│   ├── README.md                            Tối ưu chi phí lưu trữ — tổng quan
│   ├── s3-pricing-guide.md                  Bảng giá S3 — storage, requests, data transfer
│   ├── ebs-pricing-guide.md                 Bảng giá EBS — gp3 vs io2 vs st1 — tính toán
│   ├── lifecycle-rules-design.md            Thiết kế lifecycle rules — ví dụ thực tế
│   ├── intelligent-tiering-when.md          Khi nào Intelligent-Tiering tiết kiệm chi phí
│   ├── cost-allocation-tags.md              Thẻ phân bổ chi phí — phân tích theo team/project
│   └── s3-storage-lens.md                   Storage Lens dashboard — phân tích toàn tổ chức
│
├── 07-monitoring/
│   ├── README.md                            Monitoring & Observability cho AWS storage
│   ├── cloudwatch-s3-metrics.md             CloudWatch metrics cho S3 — BucketSizeBytes, etc.
│   ├── cloudwatch-ebs-metrics.md            EBS metrics — VolumeReadOps, BurstBalance, Queue
│   ├── cloudwatch-efs-metrics.md            EFS metrics — PermittedThroughput, BurstCreditBalance
│   ├── s3-server-access-logging.md          Nhật ký truy cập S3 — cấu hình và phân tích
│   ├── cloudtrail-for-storage.md            CloudTrail audit trail cho S3 API calls
│   └── cost-anomaly-detection.md            Phát hiện bất thường chi phí — cảnh báo tự động
│
├── 08-disaster-recovery/
│   ├── README.md                            Disaster Recovery — RPO/RTO cho lưu trữ
│   ├── rpo-rto-for-storage.md               RPO/RTO — ý nghĩa và thiết kế cho storage
│   ├── s3-crr-for-dr.md                     Cross-Region Replication cho DR — thiết kế
│   ├── ebs-snapshot-strategy.md             Chiến lược snapshot EBS — tần suất, retention
│   ├── aws-backup-service.md                AWS Backup — backup tập trung, vault lock
│   └── backup-vault-lock.md                 Vault lock — WORM cho backup, compliance
│
├── 09-storage-gateway/
│   ├── README.md                            Storage Gateway & Hybrid Cloud — tổng quan
│   ├── file-gateway.md                      File Gateway — NFS/SMB → S3, use cases
│   ├── volume-gateway.md                    Volume Gateway — iSCSI block storage → S3
│   ├── tape-gateway.md                      Tape Gateway — Virtual Tape Library thay thế băng vật lý
│   ├── datasync.md                          DataSync — đồng bộ tự động on-premises ↔ AWS
│   └── direct-connect-vs-vpn.md             Direct Connect vs VPN cho hybrid storage
│
├── 10-snow-family/
│   ├── README.md                            Snow Family & di chuyển dữ liệu quy mô lớn
│   ├── snowcone.md                          Snowcone — 8TB, nhỏ gọn, edge computing
│   ├── snowball-edge.md                     Snowball Edge — 80TB storage / 42TB compute
│   ├── snowmobile.md                        Snowmobile — 100PB, container, datacenter migration
│   ├── snow-vs-datasync.md                  Chọn Snow Family hay DataSync — decision tree
│   └── opshub-management.md                 OpsHub — giao diện quản lý Snow devices
│
├── 11-fsx/
│   ├── README.md                            FSx — Advanced File Systems, tổng quan
│   ├── fsx-for-windows.md                   FSx Windows — Active Directory, SMB, DFS
│   ├── fsx-for-lustre.md                    FSx Lustre — HPC, ML training, tích hợp S3
│   ├── fsx-for-netapp-ontap.md              FSx NetApp ONTAP — enterprise migration, dedup
│   ├── fsx-for-openzfs.md                   FSx OpenZFS — snapshots tức thì, clones
│   └── fsx-comparison.md                    So sánh FSx variants — chọn đúng loại
│
├── 12-interview-prep/
│   ├── README.md                            Tổng quan chuẩn bị phỏng vấn AWS storage
│   ├── INTERVIEW_GUIDE.md                   Top 20 câu hỏi phỏng vấn AWS storage — Q&A đầy đủ
│   ├── system-design-scenarios.md           Tình huống thiết kế: data lake, backup, DR
│   ├── star-stories.md                      Template câu chuyện STAR — storage incidents
│   ├── cost-optimization-questions.md       Câu hỏi về tối ưu chi phí lưu trữ
...
| **S3 Fundamentals**         | 01-s3-fundamentals/ (6 files)   | 🚧         | Cần tạo        |
...
| **Disaster Recovery**       | 08-disaster-recovery/ (5 files) | 🚧         | Cần tạo        |
| **Storage Gateway**         | 09-storage-gateway/ (5 files)   | 🚧         | Cần tạo        |
...
**Phiên Bản:** 1.0 (README + INDEX đã tạo)
**Trạng Thái:** ✅ README.md | ✅ INDEX.md | 🚧 Các phần chi tiết đang triển khai
